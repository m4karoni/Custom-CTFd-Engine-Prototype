...
def createPort(challenge,flag,team,port):
	...
	system(f"docker run -d --rm --name web_{port.url} -e FLAG=\"{generateFlag(flag,team)}\" -p {port.number}:80 --network=\"custom-ctfd-engine-test_default\" {challenge.docker_name}")
	...
	system(f"""docker stop web_{port.url} -t 1800 && docker exec custom-ctfd-engine_db-test_1 bash -c "mysql -uroot -pctfd ctfd -e 'delete from ports where url = \\"{port.url}\\";'" &""")
